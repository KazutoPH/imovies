          ...
          <div className="flex flex-col gap-5">
            {navList.map((nav, i) => (
              <div key={i}>
                <div
                  onClick={() => {
                    if (nav.name !== selectedType) setSelectedType(nav.name);
                ...
                >
                  <div className="flex items-center">
                    {nav.icon}
                    <p className="font-semibold text-xl text-white whitespace-nowrap ml-2">
                      {nav.name}
                    </p>
                  </div>
                  <div
                    className={`ml-2 ${
                      nav.name === selectedType ? " -rotate-180" : ""
                    } transition`}
                  >
                  ...
                  {selectedType === nav.name && (
                    ...
                    >
                      {nav.category.map((category, x) => (
                        <div
                          onClick={() => setSideNavPress(false)}
                          className="hover:bg-grey ease-in transition hover:cursor-pointer p-2 ml-7"
                          key={x}
                        >
                          <Link
                            href={
                          ...
                          >
                            <p
                              key={x}
                              className=" font-medium text-base text-white whitespace-nowrap"
                            >
                              {category.name}
                            </p>
                          </Link>
