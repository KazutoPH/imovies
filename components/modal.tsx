...
function Modal() {
  ...
  const fetchData = async () => {
    ...
    const result = await getTrailer(search?.toString()).then((data) =>
      setTrailer(data)
    );
    console.log(trailer);
  };
  ...
  return (
    <>
      {show && (
        <div
          className={` darkbg flex items-center justify-center flex-col z-[60] relative`}
        >
          <div className=" h-auto w-[95%] aspect-video sm:w-4/5 p-2 bg-dark flex flex-col gap-1 relative z-[100]">
            <div
              className="flex self-end hover:scale-110 active:scale-100 hover:cursor-pointer transition"
              onClick={() => {
